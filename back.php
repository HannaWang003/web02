...
<body>
	<div id="all">
		<div id="title">
			<?= date("m 月 d 日 l") ?> | 今日瀏覽: <?= $Total->find(['date' => date("Y-m-d")])['total'] ?> | 累積瀏覽: <?= $Total->sum('total') ?>
			...
			<div class="hal" id="main">
				<div>
					...
					<span style="width:18%; display:inline-block;">
						...
						if (isset($_SESSION['user'])) {
						?>
							<span>歡迎，<?= $_SESSION['user'] ?></span>
							<button onclick="logout()">登出</button>
							<?= ($_SESSION['user'] == "admin") ? "<button onclick=location.href='back.php'>管理</button>" : "" ?>
						<?php
						} else {
						?>
